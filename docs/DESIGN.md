...
**여섯 가지(나이·사는 곳·가족·집·소득·재산)만 답하면 흩어진 청약·공공임대 공고 중 「지금 신청할 수 있는 것」과 「예상 순위·가점」을 보여준다.**
...
- **템플릿 = 법령 기본값.** 공고문 PDF를 읽기 전에도 행복주택 청년이면 「만 19~39세·미혼·본인 무주택·소득 100%(1인 120%)·자산 2억 5,100만」으로 **첫날부터 판정**한다(`basis=template`, 화면에 「공고문 대신 법에 정한 기본 기준으로 봤어요」).
- **해당지역·신청 가능 지역(2026-09-25 정정).** 해당 주택건설지역은 특별·광역·특별자치시와 제주는 시·도, 도는 시·군 단위(`checks.sameLocal`). 분양은 수도권·충청권·부울경·대구경북·호남·강원·제주 권역 밖이면 신청 불가로 본다(`lib/place.ts BLOCS`, 예외는 공고문 확인 안내).
- **입력 항목(2026-09-25 추가).** 혼인 상태 「혼자(이혼·사별)」(`solo`, 1인 가구), 혼인신고 연도(`marriedYear`), 만 6세 이하 자녀 수(`youngChildren`), 부동산(`property`), 배우자 통장(`spouseAccountMonths`). 화면은 자녀가 없으면 아기·6세 이하 질문을, 만 60세 이상이면 부모 부양을, 만 40세 이상이면 대학생 질문을 묻지 않고 「아니요」로 둔다. 예/아니요는 모두 긍정형, 「모름」은 따로 저장하지 않고 unknown으로 둔다. 추천 질문(`suggestAsks`)은 이미 답한 칸을 다시 묻지 않는다.
- **다음 행동.** 상세 화면의 「지금 할 일」은 `lib/howto.ts`(접수처·방법·준비물, 예치금 부족분)에서 만든다. 접수처 주소는 공식 포털 첫 화면만 건다.
