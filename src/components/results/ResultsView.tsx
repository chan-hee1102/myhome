...
import { badgeStatus, dayText, isUrgent, rankTone, reasonLine, scoreLine, VERDICT, verdictKey } from "./verdict";
    ...
    >
      ...
      >
        ...
        <div className="flex min-w-0 items-baseline gap-2 md:block">
          ...
          <p className="whitespace-nowrap text-[14px] font-medium text-sub tabular md:mt-1 md:text-[15px]">{day.small}</p>
        </div>
        ...
        <div className="min-w-0">
          <p className="truncate text-[15px] font-medium text-muted">
            ...
            {r.a.sample && <span className="ml-1.5 text-muted">예시</span>}
          </p>
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 md:hidden">
            ...
            {r.best.rank && r.verdict !== "no" && r.phase !== "closed" && <span className={`text-[16px] font-bold ${rankTone(r.verdict)}`}>{r.best.rank.label}</span>}
          </div>
          {groups.length > 1 && (
            <ul className="mt-2.5 flex flex-wrap gap-x-4 gap-y-1" aria-label="공급 대상별 결과">
              {groups.map((g) => {
                ...
                return (
                  <li key={g.id} className={`inline-flex items-center gap-1.5 text-[15px] ${s === "ok" ? "font-semibold text-ink" : s === "maybe" ? "text-maybe-ink" : "text-muted"}`}>
                    <WinGlyph state={s} />
        ...
        <div className="hidden flex-col items-end gap-1 text-right md:flex">
          ...
          {r.best.rank && r.verdict !== "no" && r.phase !== "closed" && (
            <span className={`mt-1 text-[19px] font-bold leading-tight tracking-[-0.02em] ${rankTone(r.verdict)}`}>{r.best.rank.label}</span>
          )}
          {score && r.verdict !== "no" && r.phase !== "closed" && <span className="data text-[14px] md:text-[15px] text-sub">{scoreLine(score)}</span>}
        </div>
      </Link>
    </motion.li>
  );
...
function ChangeNote({ counts, manual }: { counts: ReturnType<typeof countVerdicts>; manual: number }) {
  ...
  const text =
    settled > 0
      ? `방금 답한 내용으로 ${settled}건이 정해졌어요.${dOk > 0 ? ` 신청 가능이 ${dOk}건 늘었어요.` : ""}`
      : dOk !== 0
        ? `방금 답한 내용으로 신청 가능이 ${dOk > 0 ? `${dOk}건 늘었어요` : `${-dOk}건 줄었어요`}.`
        : counts.maybe > 0
          ? manual >= counts.maybe
            ? `결과는 그대로예요. 남은 ${counts.maybe}건은 공고문에서 직접 확인해야 해요.`
            : `결과는 그대로예요. 남은 ${counts.maybe}건은 다른 질문에 더 답해야 정해져요.`
          : "결과는 그대로예요.";
  return (
    <motion.p
      ...
      className="mt-6 border-y border-line py-3 text-[16px] font-semibold text-ink"
      role="status"
    >
      {text}
    </motion.p>
      ...
      <main className="wrap-app pb-20 pt-24 md:pb-28 md:pt-28">
        ...
        <section className="mt-8 grid gap-8 lg:grid-cols-12 lg:gap-8">
          <div className="min-w-0 lg:col-span-8">
            <div className="section-head">
              <span>
                {empty ? "전체 공고" : "내 조건으로 본 공고"}
                {hydrated && <span className="ml-2 text-[15px] font-medium text-muted">{results.length}건</span>}
              </span>
            </div>
            {!hydrated ? (
              ...
              <div className="mt-6">
                <h1 className="t-h1">{br("진행 중인 공고를 | 먼저 둘러보세요")}</h1>
              ...
              <>
                ...
                <dl className="mt-6 flex flex-wrap items-end gap-x-10 gap-y-5">
                  <div>
                    <dt className="text-[15px] font-semibold text-sub">신청 가능</dt>
                  ...
                  {(
                    ...
                    <div key={k}>
                      <dt className="text-[15px] font-semibold text-sub">{k}</dt>
          ...
          <section className="mt-8 rounded-[4px] bg-page px-5 py-4 ring-1 ring-inset ring-line-strong md:px-6">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2">
              ...
              {asks.slice(1, 4).map((a) => (
                <Link key={a.topic} href={`/check?topic=${a.topic}`} className="inline-flex h-9 items-center text-[16px] font-semibold text-ink underline decoration-line-strong underline-offset-4 hover:decoration-ink">
        ...
        <div className="sticky top-16 z-20 -mx-5 mt-10 bg-wash px-5 pt-2 md:-mx-8 md:px-8">
            ...
            {TABS.map((t) => {
              ...
              return (
                <button
                  ...
                  className={`relative shrink-0 whitespace-nowrap pb-3 pt-1 text-[16px] font-semibold transition-colors ${on ? "text-ink" : "text-muted hover:text-ink"}`}
                >
          ...
          <div className="flex items-center justify-between gap-3 py-3">
            <div role="group" aria-label="유형" className="flex h-11 shrink-0 items-center rounded-[10px] bg-well p-1">
              {(
                [
                  ["all", "전체"],
                  ["rent", "임대"],
                ...
                <button
                  ...
                  className={`h-9 whitespace-nowrap rounded-[8px] px-3.5 text-[16px] font-semibold transition-colors ${kind === k ? "bg-page text-ink ring-1 ring-line-strong" : "text-sub hover:text-ink"}`}
                >
                  {l}
                </button>
              ))}
            </div>
            <label className="flex shrink-0 items-center gap-2 text-[15px] font-medium text-sub">
              <span className="sr-only md:not-sr-only">정렬</span>
              <select
                value={sort}
                onChange={(e) => setParam("sort", e.target.value, "match")}
                className="h-10 rounded-[10px] bg-page px-3 text-[16px] font-semibold text-ink ring-1 ring-inset ring-line-strong outline-none focus:ring-2 focus:ring-brand"
              >
        ...
        <p className="t-small mt-8 text-muted">참고용 결과예요. 최종 자격과 순위는 공급기관 심사로 정해져요.</p>
      </main>
    </div>
  );
}
