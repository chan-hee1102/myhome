    ...
    <>
      ...
      <main className="wrap pb-20 pt-24 md:pb-28 md:pt-32">
        ...
        <div className="mt-8 lg:grid lg:grid-cols-12 lg:gap-8">
          <article className="min-w-0 lg:col-span-8">
            ...
            <section id="sources" className="mt-14 md:mt-16">
              <h2 className="t-h3 text-ink">출처</h2>
              <ol className="mt-4 space-y-2">
                {g.sources.map((s) => (
                  <li key={s.url} className="t-small text-sub">
                    <a href={s.url} target="_blank" rel="noopener noreferrer" className="underline decoration-line-strong underline-offset-4 hover:text-ink hover:decoration-ink">
                      {s.label}
                    </a>
                  </li>
                ))}
            ...
            <div className="sticky top-24 space-y-4">
              <nav aria-label="목차" className="border-t border-ink pt-4">
                ...
                <ol className="mt-3 space-y-2">
                  {g.sections.map((s) => (
                    <li key={s.id}>
                      <a href={`#${s.id}`} className="t-small block text-sub hover:text-ink hover:underline">
                        {s.h2}
                      </a>
                    </li>
                  ))}
                  {g.faq.length > 0 && (
                    <li>
                      <a href="#faq" className="t-small block text-sub hover:text-ink hover:underline">
                        자주 묻는 질문
                      </a>
                    </li>
                  )}
        ...
        <section className="mt-20 md:mt-28">
          <h2 className="t-h2">함께 보면 좋은 기준</h2>
          <div className="mt-8">
