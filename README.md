...
- **문법은 「공고문 서식」.** 카드 그림자 대신 1px 선(카드는 `line-strong`, 검은 선은 표 머리 아래만). 섹션은 굵은 소제목(`.section-head`)이나 `t-h2` 한 줄 + 설명 한 줄 — 작은 회색 눈썹 라벨·「라벨 + 두 줄 제목」 틀은 쓰지 않는다(2026-09-26 「안 보이고 AI 티 난다」 피드백). 왼쪽 정렬 12열, 가운데 정렬 제목·색 eyebrow·파스텔 타일·그라디언트·폰 목업은 쓰지 않는다.
